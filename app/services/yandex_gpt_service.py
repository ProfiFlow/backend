...
class YandexGPTMLService:
    ...
    async def analyze_employee_activity(
        ...
        response_obj = await self._call_llm_structured(
    ...
    async def rate_employee_performance(
        self,
        employee_id: str,
        sprint_stats: Dict[str, Any],
        tasks: List[Dict[str, Any]],
        previous_sprint_stats: Optional[Dict[str, Any]] = None,
    ) -> Dict[str, Any]:
        ...
        completed_task_titles = [
            t.get("title", "Без названия") for t in tasks if t.get("is_completed")
        ]
        task_titles_str = (
            "\n".join([f"- {title}" for title in completed_task_titles])
            or "Завершенных задач нет."
        )
        ...
        response_obj = await self._call_llm_structured(
            system_prompt, user_prompt, response_model=RatingResponse
        )
        # Return dict matching expected format
        return {"rating": response_obj.rating, "explanation": response_obj.explanation}

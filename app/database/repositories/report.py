...
class ReportRepository:
    ...
    async def save_or_update_sprint_report(self, **kwargs):
        ...
        if not report:
            report = SprintReportDB(
                ...
                average_task_completion_time=kwargs[
                    "average_task_completion_time"
                ].current,
                activity_analysis=kwargs["activity_analysis"],
                recommendations=[
                    r.model_dump() for r in (kwargs["recommendations"] or [])
                ],
            )
            ...
            report.average_task_completion_time = kwargs[
                "average_task_completion_time"
            ].current
    ...
    async def get_previous_sprint_report(
        self, user_id: int, tracker_id: int, sprint_start_date
    ):
    ...
    async def get_sprint_report_by_id(
        self, user_id: int, tracker_id: int, sprint_id: int
    ):
        q = await self.session.execute(
            select(SprintReportDB)
            .where(SprintReportDB.user_id == user_id)
...
class TeamReportRepository:
    ...
    async def save_or_update_team_sprint_report(
        self,
        tracker_id: int,
        sprint_id: int,
        sprint_start_date,
